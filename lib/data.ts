...
import { clearMemberSession, getCurrentMemberId, setMemberSession } from "@/lib/member-session";
import { createSupabaseServerClient } from "@/lib/supabase/server";
import type { Member, Session, SessionFormState, SessionSummary, SessionWithPlayers } from "@/lib/types";
import { summarizePlayers } from "@/lib/calculations";

const memberSelect = "id,name,login_name,phone,note,active,created_at,updated_at";
const sessionSelect =
...
export async function loginMember(_: { ok: boolean; message: string }, formData: FormData) {
  ...
  if (error || !data || !data.active || data.pin_code !== pinCode) {
    return { ok: false, message: "Thông tin đăng nhập không đúng hoặc tài khoản đã ngưng hoạt động." };
  }
...
export async function registerMember(_: { ok: boolean; message: string }, formData: FormData) {
  ...
  if (error) {
    return { ok: false, message: error.code === "23505" ? "Tên đăng nhập đã tồn tại." : error.message };
  }
...
export async function createMember(formData: FormData) {
  ...
  const { error } = await supabase.from("members").insert({
    name,
    login_name: toString(formData.get("login_name")) ? normalizeLoginName(formData.get("login_name")) : null,
...
export async function getSessions(filters?: {
  ...
  if (filters?.month) {
    sessions = sessions.filter((session) => session.date.startsWith(filters.month ?? ""));
  }

  if (filters?.memberName) {
    const query = filters.memberName.toLowerCase();
    sessions = sessions.filter((session) =>
      session.players.some((player) => player.member_name_snapshot.toLowerCase().includes(query)),
    );
  }
...
export async function getSessionDetail(id: string) {
  const supabase = createSupabaseServerClient();
  const { data, error } = await supabase.from("sessions").select(sessionSelect).eq("id", id).single();
...
function parseSessionPayload(formData: FormData) {
  ...
  const shuttlecockFee = calculateShuttlecockFee(shuttlecockUnitPrice, shuttlecockQuantity);
  ...
  assertNonNegativeFees([courtFee, shuttlecockUnitPrice, shuttlecockQuantity, drinkFee, otherFee]);
  ...
  if (players.some((player) => !player.memberName)) throw new Error("Tên người chơi là bắt buộc.");
  if (players.some((player) => player.shareCount <= 0)) throw new Error("Tập suất phải lớn hơn 0.");
...
export async function updateSessionWithPlayers(formData: FormData) {
  ...
  const paidByMember = new Map(
    ((existingPlayers ?? []) as { member_id: string | null; paid: boolean; paid_at: string | null }[])
      .filter((player) => player.member_id)
      .map((player) => [player.member_id, { paid: player.paid, paid_at: player.paid_at }]),
  );

  const { error: updateError } = await supabase.from("sessions").update(payload.session).eq("id", sessionId);
  if (updateError) throw new Error(updateError.message);

  const { error: deleteError } = await supabase.from("session_players").delete().eq("session_id", sessionId);
  if (deleteError) throw new Error(deleteError.message);

  const { error: insertError } = await supabase.from("session_players").insert(
    payload.calculatedPlayers.map((player) => {
      const paidState = player.memberId ? paidByMember.get(player.memberId) : undefined;
      return {
        session_id: sessionId,
        member_id: player.memberId,
        member_name_snapshot: player.memberName,
