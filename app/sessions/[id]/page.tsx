...
import { deleteSession, getSessionDetail, updatePlayerPaidStatus } from "@/lib/data";
    ...
    <AppShell>
      <section className="card">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <h1 className="text-xl font-black tracking-tight text-slate-950">Buổi chơi {formatDate(session.date)}</h1>
            ...
            <div className="rounded-2xl border border-slate-200 p-4" key={player.id}>
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <div className="mt-1 text-xs font-bold text-slate-500">
                    Tập suất {Number(player.share_count)} · {player.drink_shared ? "Có chia nước" : "Không chia nước"} · Phụ thu {formatCurrency(player.adjustment)}
                  </div>
                  {player.note ? <div className="mt-2 text-sm font-medium text-slate-600">{player.note}</div> : null}
                </div>
                <div className="text-right">
                  <div className="font-black text-slate-950">{formatCurrency(player.amount)}</div>
                  <div className={`mt-1 text-xs font-black ${player.paid ? "text-emerald-700" : "text-rose-600"}`}>
              ...
              {isAdmin ? (
                <form action={updatePlayerPaidStatus} className="mt-3">
                  ...
                  <input name="paid" type="hidden" value={player.paid ? "false" : "true"} />
...
function Cost({
  ...
  return (
    <div className="rounded-2xl bg-slate-50 p-4">
      ...
      <div className={`mt-1 font-black ${strong ? "text-xl text-indigo-700" : "text-slate-950"}`}>{formatCurrency(value)}</div>
