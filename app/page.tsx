  ...
  return (
    <AppShell>
      <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <MetricCard label="Còn nợ" value={formatCurrency(data.totalUnpaid)} tone="danger" />
      </section>
      ...
      <section className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
          ...
          <div className="flex flex-col gap-3">
            {data.recentSessions.length === 0 ? (
              ...
              data.recentSessions.map((session) => (
                <Link className="rounded-2xl border border-slate-200 p-4 hover:bg-slate-50" href={`/sessions/${session.id}`} key={session.id}>
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <div className="font-black text-slate-950">{formatDate(session.date)}</div>
                      <div className="mt-1 text-sm font-semibold text-slate-500">{session.playerCount} người chơi</div>
                    </div>
        ...
        <div className="card">
          <h2 className="mb-4 text-lg font-black text-slate-950">Người chơi còn nợ</h2>
          <div className="flex flex-col gap-3">
            {data.unpaidPlayers.length === 0 ? (
              <EmptyState text="Không có khoản nợ nào." />
            ) : (
              data.unpaidPlayers.slice(0, 10).map((player) => (
                <Link className="flex items-center justify-between gap-3 rounded-2xl border border-slate-200 p-3 hover:bg-slate-50" href={`/sessions/${player.sessionId}`} key={player.id}>
                  <span>
                    <span className="block font-black text-slate-950">{player.member_name_snapshot}</span>
                    <span className="block text-xs font-semibold text-slate-500">{formatDate(player.sessionDate)}</span>
                  </span>
                  <span className="font-black text-rose-600">{formatCurrency(player.amount)}</span>
                </Link>
              ))
            )}
          </div>
...
function MetricCard({ label, value, tone = "default" }: { label: string; value: string; tone?: "default" | "danger" }) {
  return (
    <div className="card">
      ...
      <div className={`mt-2 text-2xl font-black tracking-tight ${tone === "danger" ? "text-rose-600" : "text-slate-950"}`}>
        {value}
      </div>
    </div>
  );
